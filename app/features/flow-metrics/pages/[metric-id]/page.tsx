  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <CommonHeader
          currentPage="Flow Metric Detail"
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <Button
            variant="outline"
            onClick={() => router.push(`/flow-metrics-report?period=${selectedPeriod}`)}
            className="mb-4"
          >
            ← Back to Dashboard
          </Button>
          <h1 className="text-2xl font-bold text-gray-900">
            {config?.display_title || 'Metric Detail'}
          </h1>
          {config?.metric_comment && (
            <p className="text-sm text-gray-600 mt-1">{config.metric_comment}</p>
          )}
        </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">
                {calculatedMetrics.average > 0
                  ? `${calculatedMetrics.average} days`
                  : 'No data'}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">
                {calculatedMetrics.best > 0
                  ? `${calculatedMetrics.best} days`
                  : 'No data'}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-600">
                {calculatedMetrics.worst > 0
                  ? `${calculatedMetrics.worst} days`
                  : 'No data'}
              </div>
            </CardContent>
          </Card>
                    ...
                    <tbody>
                      {deals.map((deal, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          <td className="p-2">{deal.deal_id}</td>
                          <td className="p-2">
          ...
          <Card>
            <CardContent className="py-12">
              <div className="text-center text-gray-500">
                ...
                <p className="text-sm">
                  No deals found for this metric in the selected time period.
                </p>
              </div>
            </CardContent>
          </Card>
