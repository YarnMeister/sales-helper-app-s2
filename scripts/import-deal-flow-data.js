...
/**
 * Temporary script to import deal flow data from Pipedrive
 * 
...
/**
 * Insert flow data into database using raw SQL
 */
async function insertDealFlowData(flowData) {
  if (!flowData || flowData.length === 0) return;
  
  // For now, just log the data that would be inserted
  // In a real implementation, you would use a database client
  console.log(`📊 Would insert ${flowData.length} flow records for deal ${flowData[0].deal_id}`);
  
  // Example of what the SQL would look like:
  // INSERT INTO pipedrive_deal_flow_data (
  //   pipedrive_event_id, deal_id, pipeline_id, stage_id, stage_name, 
...
async function insertDealMetadata(dealMetadata) {
  ...
  console.log(`📊 Would insert metadata for deal ${dealMetadata.id}`);
  
  // Example of what the SQL would look like:
  // INSERT INTO pipedrive_metric_data (id, title, pipeline_id, stage_id, status)
  // VALUES ...
...
async function main() {
  try {
    ...
    console.log('\n✅ Import completed!');
    console.log('\n⚠️  NOTE: This is a dry run. No data was actually inserted into the database.');
    console.log('   To perform the actual import, implement the database insertion functions.');
