...
/**
 * Deal Flow Data Import Script - Database Version
 * 
 * This script imports deal flow data from Pipedrive and commits it to the database
 * for use in the Flow Metrics Report Raw Data tab.
 * 
 * Rate limiting: 40 requests per 2 seconds (Pipedrive limit)
 * Error handling: Skip failed deals, retry once, then give up
 * Progress: Basic heartbeat indicator and final summary
 * 
 * Usage: node scripts/import-deal-flow-data-with-db.js
 */
...
/**
 * Insert flow data into database using direct SQL
 */
async function insertDealFlowData(flowData) {
  if (!flowData || flowData.length === 0) return 0;
  ...
  for (const data of flowData) {
    try {
      ...
      console.log(`⚠️  Error inserting flow data for deal ${data.deal_id}: ${error.message}`);
    }
  }
  ...
  try {
    ...
    const result = await sql`
      INSERT INTO pipedrive_metric_data (
        ...
        ${dealMetadata.id},
        ${dealMetadata.title || 'Unknown'},
        ${dealMetadata.pipeline_id || 1},
        ${dealMetadata.stage_id || 1},
        ${dealMetadata.status || 'active'}
      )
    ...
    console.log(`⚠️  Error inserting metadata for deal ${dealMetadata.id}: ${error.message}`);
    return null;
  }
}
...
async function processDeal(dealId) {
  try {
    ...
    const insertedFlowRecords = await insertDealFlowData(processedFlowData);
    
    // Store deal metadata (using first event for basic info)
    const firstEvent = flowData[0];
    const dealMetadata = {
      id: firstEvent.deal_id,
      title: firstEvent.data?.title || `Deal ${firstEvent.deal_id}`,
      pipeline_id: 1,
      stage_id: firstEvent.data?.stage_id || 1,
      status: 'active'
    };
    await insertDealMetadata(dealMetadata);
    
    console.log(`✅ Deal ${dealId}: ${insertedFlowRecords} new flow records inserted`);
    return { success: true, events: insertedFlowRecords };
...
async function main() {
  try {
    console.log('🚀 Starting deal flow data import with database insertion...');
    startTime = Date.now();
    ...
    console.log('\n✅ Import completed! Data has been committed to the database.');
    console.log('📊 Check the Flow Metrics Report Raw Data tab to see the imported data.');
